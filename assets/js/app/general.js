define(["jquery", "allsite"], function ($, gen) {
  $(document).ready(function () {
    ...
    if (Modernizr.svg === false) {
      //target browsers that don't support SVG
    ...
    if (!Modernizr.input.placeholder) {
      //target browsers that doesn't support placeholder attribute
      $("[placeholder]")
        .focus(function () {
          var input = $(this);
    ...
    if (topNavObj.hasClass("nav--sticky-init")) {
      var topNavFullHeight =
        parseInt(topNavObj.height()) +
        parseInt(topNavObj.css("padding-top")) +
        parseInt(topNavObj.css("padding-bottom"));
    ...
    if (agentStr.indexOf("Trident/5.0") > -1) {
      if (agentStr.indexOf("MSIE 7.0") > -1) isCompatabilityMode = true;
    } else if (agentStr.indexOf("Trident/4.0") > -1) {
      if (agentStr.indexOf("MSIE 7.0") > -1) isCompatabilityMode = true;
    }
    if (isCompatabilityMode || debug) {
      var messageStr =
        "<p>This website will not display correctly in compatibilty mode.";
      messageStr +=
        " For more information please see <a href='https://ucl.ac.uk/indigo/design-foundation/indigo-constraints'>Indigo constraints</a></p>";

      $("body").prepend(
        "<div class='announcement-bar'>" +
          messageStr +
          "</a><a href='#' class='announcement-bar--close'>x</span>"
      );
